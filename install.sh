...
BACALHAU_INSTALLATION_ID="33b78418-718d-4c67-8e82-ba008ab476a1"

BACALHAU_METRICS_ENDPOINT="https://i.bacalhau.org"

BACALHAU_INSTALL_SCRIPT_HASH="c4371aed48fde4cf5095f46caf86d4e72514697a6e1b9eaa644c39566e6186d1"
...
addInstallationID() {
    ...
    if [ -z "$BACALHAU_INSTALLATION_ID" ] || [[ "$BACALHAU_INSTALLATION_ID" == *InstallationID* ]]; then
...
getReleaseVersion() {
    ...
    if [ -n "$BACALHAU_VERSION" ]; then
        case "$BACALHAU_VERSION" in
            ...
            *)
                ...
                local versionUrl="https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/releases/tags/${BACALHAU_VERSION}"
                local versionExists
    ...
    if [ "$PRE_RELEASE" == "true" ]; then
        ...
        bacalhauReleaseUrl="https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/releases?per_page=1"
    else
        bacalhauReleaseUrl="https://api.github.com/repos/${GITHUB_ORG}/${GITHUB_REPO}/releases/latest"
    fi
